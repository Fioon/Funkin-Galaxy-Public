...
class PauseSubState extends MusicBeatSubstate
{
	...
	public function new(x:Float, y:Float)
	{
		...
		creditsText = new FlxText(20, 15 + (32 * 1), 0, 'Composer: ${PlayState.SONG.songComposer}', 32);
		add(creditsText);

		var levelInfo:FlxText = new FlxText(20, 15 + (32 * 0), 0, PlayState.SONG.song, 32);
		add(levelInfo);

		var levelDifficulty:FlxText = new FlxText(20, 15 + (32 * 2), 0, "Difficulty: ", 32);
		levelDifficulty.text += CoolUtil.difficultyString();
		add(levelDifficulty);

		var blueballedTxt:FlxText = new FlxText(20, 15 + (32 * 3), 0, "", 32);
		...
		var chartingText:FlxText = new FlxText(20, 15 + 101, 0, "CHART EDITOR PREVIEW", 32);
		...
		for (daCoolText in [creditsText, levelInfo, levelDifficulty, blueballedTxt, practiceText, chartingText])
			daCoolText.x = FlxG.width - (daCoolText.width + 20);
		...
		FlxTween.tween(creditsText, {alpha: 1, y: creditsText.y + 5}, 0.4, {ease: FlxEase.quartInOut, startDelay: 0.1});
	...
	function startModeTimer():Void
	{
		modeFadeTween = FlxTween.tween(creditsText, {alpha: 0.0}, MODE_FADE_DURATION,
		{
			...
			onComplete: (_) -> {
				...
				creditsText.x = FlxG.width - (creditsText.width + 20);
