...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		if(onComboMenu)
		{
			...
			if (holdingObjectType == null) {
				if ((mousePos.x - comboNums.x >= 0 && mousePos.x - comboNums.x <= comboNums.width &&
					mousePos.y - comboNums.y >= 0 && mousePos.y - comboNums.y <= comboNums.height) || (mousePos.x - rating.x >= 0 && mousePos.x - rating.x <= rating.width &&
						mousePos.y - rating.y >= 0 && mousePos.y - rating.y <= rating.height))
					MouseCursors.loadCursor("grab_prepare");
				else
