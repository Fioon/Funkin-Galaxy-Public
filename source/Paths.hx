...
class Paths
{
	...
	static function cneImage(key:String, ?library:String, checkForAtlas:Bool = false, ?ext:String = "png")
	{
		if (checkForAtlas)
		{
			var atlasPath = SUtil.getPath() + getPath('images/$key/spritemap.$ext', IMAGE, library);
			var multiplePath = SUtil.getPath() + getPath('images/$key/1.$ext', IMAGE, library);
		...
		return SUtil.getPath() + getPath('images/$key.$ext', IMAGE, library);
	}
